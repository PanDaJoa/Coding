...
public class TryCatchTest : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        catch (Exception e) // 예외를 잡는다.
        {
            ...
            Debug.Log(message: "예외가 발생했습니다.");
            Debug.LogError(message: $"예외: {e.Message}"); // Message: 예외에 대한 설명
        }
        finally
        {
            // 예외가 발생 또는 발생하지 않아도 무조건 처리될 구문을 묶어주는데 사용한다.
            Debug.Log(message: "Finally");
        }
        
        // try-catch를 남용하면 프로그램 성능이 저하되므로 꼭 필요한 상황이 아니면 안쓰는게 좋다.
        // 예외가 예상되는 코드에는 방어 다른 메커니즘(방어 코드: 조건문 사용)을 사용하는 것이 좋다.
        // 단점:
        // 1. 성능 저하 (스택 언롤링)
        // 2. 예외 객체 생성
        // 3. 로직의 복잡성 (유지보수 저하)
